...
> 产品、视觉、交互与开发者体验设计见同级 `DESIGN.md`（如项目需要）。

**仓库真值入口固定为根目录的 `AGENTS.md`、`CLAUDE.md`、`steering/`。设计系统入口为 `DESIGN.md`（按需使用）。**
**Harness 角色**：Harness 在 IDE 中通过 `/harness-init` 检测项目现状、生成最少但更正确的根级真值与协议文件。本文件只定义协作原则，不把任何第三方工作流写成唯一事实源。

---

## 与 AGENTS.md / DESIGN.md 的分工
...
所有项目概览、目录结构、端口、脚本、模块表等可核对事实，均以根级 `AGENTS.md` 为唯一真值。本文件不重复罗列。视觉、交互、品牌、内容语气和体验反模式写入 `DESIGN.md`，不要混入本协议文件。
...
Harness 可以根据 `/harness-init` 的检测结果，在 `AGENTS.md` 或 `.harness/runs/<run-id>/recommendations.md` 中生成第三方工作流适配建议。使用原则：
