...
## 进程管理陷阱（2026-08-23 批量事故，血泪教训）

批量渲染时"进程管理"是最容易翻车的地方，这次一次踩了三个坑：

**坑 1：watchdog 重复启动（16 个实例并行）**
- 症状：watchdog 用 wmic/ps 检测"管线是否在跑"，但 subprocess.Popen 启动的子进程
  检测不到 → 每 60s 重复启动一个批处理 → 16 个实例并行抢同一批镜头
- 修法：**用 PID 标记文件**（启动时写 pid 文件，退出时删；检测时读文件 + 查进程存活），
  不用 wmic/ps 模糊检测
- 教训：监督脚本的"进程是否存活"判断必须可靠，否则监督本身变成灾难

**坑 2：taskkill 误杀**
- 症状：`taskkill //F //IM python.exe` 把所有 python 全杀了（差点杀掉 ComfyUI 主进程）
- 修法：**精确按 PID 杀**，绝不按镜像名批量杀；杀之前先 psutil 列清单确认
- 教训：Windows 杀进程要认准 PID，ComfyUI（占 6.8GB 内存的 python）是守护对象

**坑 3：渲染超时无预案**
- 症状：comfy CLI 单次渲染超时 3600s（竖屏 2K + 22 步 + UltraSharp 4x 后处理太慢），
  每次等 1 小时才知道失败，3 小时只跑 3-5 个镜头
- 修法：**先小样测速**（跑 1 个镜头计时）再定批量参数；分辨率/后处理链路过重时降配
- 教训：批量前先估算单镜头耗时，超时阈值要匹配实际耗时（2-3 倍余量）

**批量渲染启动清单（动手前必查）**
- [ ] 单镜头耗时实测（先跑 1 个计时）→ 据此定超时阈值和批量策略
- [ ] 确认没有残留 run_pipeline 进程（psutil 精确查）
- [ ] watchdog/监督脚本用 PID 文件标记，不用 wmic/ps 模糊检测
- [ ] 杀进程只按 PID，不按镜像名
- [ ] 显存/内存开工前检查（system_stats）
- [ ] 先跑通 1 个镜头（含后处理）再放批量
